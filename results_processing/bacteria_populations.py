...
def bacteria_populations(DATA_DIR, SAVE_DIR, LOC, num_frames, show_plots, save_plots):
    batches = []
    ...
    for test_num in tests:
        ...
        r_gt_masses = np.sum(img_array[..., 0], axis=(1, 2))
        g_gt_masses = np.sum(img_array[..., 1], axis=(1, 2))
        r_pd_masses = np.sum(p_img_array[..., 0], axis=(1, 2))
        g_pd_masses = np.sum(p_img_array[..., 1], axis=(1, 2))
        ...
        plt.plot(r_gt_masses, label="Test batch {}: true red".format(test_num), color="red", linestyle=style[1], alpha=.5)
        plt.plot(g_gt_masses, label="Test batch {}: true green".format(test_num), color="green", linestyle=style[1], alpha=.5)
        plt.plot(r_pd_masses, label="Test batch {}: predicted red".format(test_num), color="red", linestyle=style[1])
        plt.plot(g_pd_masses, label="Test batch {}: predicted green".format(test_num), color="green", linestyle=style[1])
