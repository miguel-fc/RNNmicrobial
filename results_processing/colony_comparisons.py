...
def extract_colony_eccentricity(im, color=None):
    """ Get eccentricity of colonies of given color in image """
    t_im = grab_color(im, color)
    labels = extract_colony_labels(t_im)
    areas = [spot["eccentricity"] for spot in measure.regionprops(labels)]
    return np.array(areas)
...
def track_colonies(ims, color=None, method="watershed", min_size=10):
    ...
    # Check for colonies that are too small through all time
    for val in np.unique(t_labels):
        count = np.sum(t_labels == val)
        if count < min_size:
            t_labels[t_labels == val] = 0
